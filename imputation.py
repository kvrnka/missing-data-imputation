...
def simple_imputation(df_incomplete, num_strategy='mean'):
    """
    Заполняет пропуски в датафрейме с помощью среднего или медианы.
...
def knn_imputation_numeric(df_incomplete, n_neighbors=5):
    """
...
def knn_imputation_gower(df_incomplete, n_neighbors=5):
    df = df_incomplete.copy()
...
def mice_imputation_hybrid(df_incomplete, max_iter=10, random_state=42):
...
def missforest_imputation(df_incomplete, max_iter=10, n_estimators=100, random_state=42):
    ...
    # LabelEncoder для категориальных (с сохранением NaN)
    encoders = {}
    for col in cat_cols:
        le = LabelEncoder()
        not_null = df[col].notna()
...
IMPUTATION_METHODS = {
    ...
    "KNN numeric": knn_imputation_numeric,
    "KNN gower": knn_imputation_gower,
    "KNN hybrid": knn_imputation_hybrid,
    "MICE": mice_imputation,
    "MICE hybrid": mice_imputation_hybrid,
